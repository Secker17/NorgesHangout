...
export default {
  async execute(interaction) {
    ...
    if (ticketChannel && ticketChannel.isTextBased()) {
      const perms = ticketChannel.permissionsFor ? ticketChannel.permissionsFor(client.user) : null;
      if (!perms || !perms.has('SendMessages') || !perms.has('ViewChannel')) {
        results.push(`Kan ikke rydde ticket-kanal ${config.TICKET_CHANNEL_ID}: mangler Send/View permissions.`);
      } else {
    ...
    if (welcomeChannel && welcomeChannel.isTextBased()) {
      const perms = welcomeChannel.permissionsFor ? welcomeChannel.permissionsFor(client.user) : null;
      if (!perms || !perms.has('SendMessages') || !perms.has('ViewChannel')) {
        results.push(`Kan ikke rydde velkomst-kanal ${config.WELCOME_CHANNEL_ID}: mangler Send/View permissions.`);
      } else {
        ...
        if (infoMsg) {
          try { if (perms.has('ManageMessages')) await infoMsg.pin(); } catch (e) { /* ignore */ }
